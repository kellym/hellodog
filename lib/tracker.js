...
class Tracker {
  ...
  _track (fn, done) {
    const transactions = state.create(new Map());
    if (done) {
      fn((...args) => {
        state.reset();
      ...
      return new Promise((resolve, reject) => {
        fn(
          this._promise(transactions, resolve),
          this._promise(transactions, reject)
        );
      });
    }
  }

  _promise (transactions, done) {
    return (...args) => {
      state.reset();
      const recording = Array.from(transactions.values());
      args.push(recording);
  ...
  _time () {
    const time = process.hrtime(this.base_hrtime);
  ...
  _startTrackingSocket (socket, opts) {
    if (!socket._connection) {
      socket._connection = info.connection(opts);
    }

    // and start listening to the necessary events to track
    // incoming data
    this._listen(socket);
  }
  ...
  _listen (socket) {
    ...
    // a TLS stream doesn't reveal itself to be encrypted, and a
    // decrypted stream sets the `encrypted` property to true, so we'll
    // try to track the stream we should discard by adding a property
  ...
  _wrapMethod (object, method, fn) {
    ...
    const wrapped = function (...args) {
      return fn.call(self, original, this, args);
    };
    merge(wrapped, original);
    wrapped._unwrap = function () { object[method] = original; };
  ...
  _socket (originalMethod, socket, args) {
    let returnedSocket = originalMethod.apply(socket, args);
    if (returnedSocket === undefined) returnedSocket = socket;
    this._startTrackingSocket(returnedSocket, args);
    return returnedSocket;
  }

  // wrapper for socket.connect
  _connect (original, socket, args) {
    this._startTrackingSocket(socket, args);
